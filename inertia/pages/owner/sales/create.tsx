...
export default function SellPage({ products, filters }: Props) {
  const breadcrumbs = [{ title: 'Inicio', href: urlFor('dashboard') }, { title: 'Vender' }]
    ...
    <OwnerLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="space-y-8">
          ...
          <div className="bg-card rounded-2xl border p-4 flex items-center justify-between shadow-sm h-12">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <p className="text-xl font-black leading-none">{products.meta.total}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center pb-10">
          <Pagination meta={products.meta} baseUrl={urlFor('sales.create')} queryParams={filters} />
        </div>
      </div>
    </OwnerLayout>
  )
